# SharePoint Online DOCX Version History Cleanup Script
# Searches for DOCX files and deletes versions older than 2024
...
param(
    ...
    [string]$TenantId,
    [int]$RetryAttempts = 1,
    [int]$MaxFiles = 4000,            # Limit files per run (increased from 500)
    ...
    [int]$MinFileSizeMB = 0,        # Only process DOCX files larger than this (reduced from 100)
    [int]$MaxFileSizeMB = 20,      # Only process DOCX files smaller than this
...
$logPath = Join-Path $LocalTempPath "docx_version_cleanup_log.txt"
$processedFilesPath = Join-Path $LocalTempPath "processed_docx_files.txt"
$spaceSavingsReportPath = Join-Path $LocalTempPath "docx_space_savings_report.txt"
...
function Get-DOCXFilesUsingPaginatedSearch {
    Write-Log "Searching for DOCX files using paginated SharePoint Search..."
    
    try {
        $minSizeBytes = $MinFileSizeMB * 1024 * 1024
        $maxSizeBytes = $MaxFileSizeMB * 1024 * 1024
        $searchQuery = "FileExtension:docx AND Size>=$minSizeBytes AND Size<=$maxSizeBytes"
        
        Write-Log "Search Query: $searchQuery"
        Write-Log "Looking for DOCX files between $MinFileSizeMB MB and $MaxFileSizeMB MB..."
        ...
        if ($docxFiles.Count -eq 0) {
            Write-Log "No DOCX files found matching size criteria" "WARN"
            return @()
        }
        
        # Remove duplicates (in case of overlapping results)
        $docxFiles = $docxFiles | Sort-Object ServerRelativeUrl -Unique
        
        # Trim to MaxFiles if needed
        if ($docxFiles.Count -gt $MaxFiles) {
            $docxFiles = $docxFiles | Select-Object -First $MaxFiles
        }
        
        # Sort by size (largest first) for maximum impact
        $docxFiles = $docxFiles | Sort-Object SizeMB -Descending
        
        Write-Log "Found $($docxFiles.Count) unique DOCX files totaling $([math]::Round(($docxFiles | Measure-Object SizeMB -Sum).Sum, 2)) MB"
        
        # Show top 10 largest files for context
        if ($docxFiles.Count -gt 0) {
            Write-Log "Top 10 largest DOCX files:"
            $docxFiles | Select-Object -First 10 | ForEach-Object {
        ...
        return Get-DOCXFilesUsingLibraryEnumeration
    }
}

function Get-DOCXFilesUsingLibraryEnumeration {
    ...
    try {
        ...
        foreach ($item in $allFiles) {
            try {
                ...
                # Check if it's a DOCX file within size range
                if ($fileName -like "*.docx" -and $fileSize -ge $minSizeBytes -and $fileSize -le $maxSizeBytes) {
                    $fileInfo = [PSCustomObject]@{
                        Name = $fileName
        ...
        Write-Log "Found $($docxFiles.Count) DOCX files via library enumeration totaling $([math]::Round(($docxFiles | Measure-Object SizeMB -Sum).Sum, 2)) MB"
...
function Process-DOCXFileVersions {
    param([object]$File)
...
function Generate-SpaceSavingsReport {
    ...
    $reportContent = @()
    $reportContent += "SharePoint DOCX Version History Cleanup - Space Savings Report"
    ...
    if ($script:ProcessedFilesList.Count -gt 0) {
        $reportContent += "DOCX FILES PROCESSED:"
        $reportContent += "-" * 50
        $reportContent += ""
    ...
    $reportContent += "-" * 50
    $reportContent += "Total DOCX Files Processed: $($script:FilesProcessed)"
...
function Show-Progress {
    ...
    Write-Host "PROGRESS: $Current/$Total DOCX files ($percentComplete%)" -ForegroundColor Yellow
...
# Main execution
Write-Log "Starting SharePoint DOCX version history cleanup..."
...
try {
    ...
    # Search for DOCX files using paginated approach
    Write-Log "Starting DOCX file discovery..."
    $docxFiles = Get-DOCXFilesUsingPaginatedSearch
    
    if ($docxFiles.Count -eq 0) {
        Write-Log "No DOCX files found matching criteria" "WARN"
        exit
    }
    ...
    foreach ($file in $docxFiles) {
        ...
        try {
            $result = Process-DOCXFileVersions -File $file
    ...
    Write-Log "Total DOCX Files Processed: $($script:FilesProcessed)"
